...
(function ($) {
    ...
    attach: function (context, settings) {
      ...
      $('.media-group').each(function(i) {
        if ($(this).children().length > 1) {
            var maxheight = 0;
            $(this).children('.media-entity').each(function(ind, elem) {
              var childheight = $(elem).height();
              maxheight = childheight > maxheight? childheight : maxheight;
            });

            $('.group-image-caption').css('padding-left','10%');
          if ($(this).children('.rotatorNav').length === 0) {
            $(this).prepend('<img class="loading" style="width:100%;height:0;padding-bottom:'+ (maxheight > 250? maxheight +'px':'calc(59% + 10em)') +';" src="/sites/all/themes/custom/bootdemo/images/dummy.gif">'); //calc(59% + 10em)
            //$(this).prepend('<div class="rotatorNav"><div class="rotatorButtons"><span class="rotatorCount"></span><button class="rotatorPrev" onClick="virtualPV(jQuery(this).get(0));"><span>Previous</span></button><button class="rotatorNext" onClick="virtualPV(jQuery(this).get(0));"><span>Next</span></button></div><div class="slide-pager slide-pager-' + i + '"></div></div>');
          ...
          var article_image_rotator = $(this).cycle({
            ...
            after: function(curr, next, opts) {
              ...
              if (opts.slideCount != 1) {
                ...
                var caption = $(this).find('.group-image-caption').height();
                caption = (caption == null) ? -20 : caption;
                var computeTop = (total-caption) - 10;*/
              ...
              // $('.group-media-block').height($('.rotatorSlide').outerHeight()+60);
            }
          });
      ...
      /*$('.arrow-left, .rotatorPrev').click(function(){virtualPV($(this).get(0));});
      $('.arrow-right, .rotatorNext').click(function(){virtualPV($(this).get(0));});*/
      $(window).resize(function() {
        $('.media-group').each(function(i) {
          /*total = $(".media-current-slide .group-image-frame").height();
          caption = $(".media-current-slide .group-image-caption").height();
          caption = (caption == null) ? -20 : caption;
          var computeTop = (total-caption) - 10;*/
          var imgresponsive = $(this).find('.media-current-slide .img-responsive');
          var imgiframe = $(this).find('.media-current-slide .file-video');
          var computeTop = 10;
          if (imgresponsive.length > 0) {
            computeTop = imgresponsive.height() + 10;
            $(this).find('.rotatorCount').css('top', computeTop);
          } else {
  ...
  Drupal.behaviors.advertorial_image_rotator = {
    attach: function (context, settings) {
      $('.rotator-advertorial .view-content').each(function(i) {
        if ($(this).children().length > 1) {
          if($(this).children('.rotatorNav').length === 0) {
           // $(this).prepend('<img class="loading" style="width:100%; height:0;padding-bottom: calc(59% + 10em);" src="/sites/all/themes/custom/bootdemo/images/dummy.gif" >');
            $(this).prepend('<div class="rotatorNav"><div class="rotatorButtons"><span class="rotatorCount"></span><button class="rotatorPrev"><span>Previous</span></button><button class="rotatorNext"><span>Next</span></button></div><div class="slide-pager slide-pager-' + i + '"></div></div>');
          }
          var advertorial_image_rotator = $(this).cycle({
            slideExpr: '.rotator-media',
            containerResize: 1,
            timeout: 6000,pause: true,
            width: '100%', fit: 1,
            pager: '.slide-pager-' + i,
            slideResize: 0,
            prev: '.rotatorPrev', next: '.rotatorNext',
            after: function(curr, next, opts) {
              if (opts.slideCount != 1) {
                $('.rotatorNav').show();
                $('.rotatorCount').html((opts.currSlide + 1) + '/' + opts.slideCount);
                slideIndex = opts.currSlide;
                nextIndex = slideIndex + 1;
                prevIndex = slideIndex - 1;
                if (slideIndex == opts.slideCount - 1) nextIndex = 0;
    ...
    attach: function (context, settings) {
      ...
      $('.navbar-toggle').click(function() {
        ...
        var menu = $('.navbar-collapse');
				var origOffsetY = menu.offset().top < 165? menu.offset().top : 111;
        //console.log('heighto', document.querySelector('.navbar-header').clientHeight + 1 );

        var topheaderheight = $('header#navbar').css('height');

        var isDesktop = !navigator.userAgent.match(/(iPad|iPhone|iPod|android)/i);
        //$('body > .highlighted').css('padding-top', topheaderheight);
				...
				sticky_nav = function() {
					if ($(window).width() > 740){
						if ($(window).scrollTop() >= origOffsetY) {
								...
								$('.highlighted').addClass('sticky-nav-padding');

                if ( $('body').hasClass('article-type-longform') ) {
                  $('header#navbar').css('position','fixed');
                }
						} else {
								$('.navbar').removeClass('navbar-fixed-top');
								$('.navbar-header').removeClass('hidden');
								$('.highlighted').removeClass('sticky-nav-padding');
            ...
            if ((document.getElementsByTagName("body")[0].scrollTop <= parseFloat(topheaderheight)) && isDesktop) {
              $('.navbar-header, .navbar-header h1.site-name, .navbar-header.has-category-term .web-category-name').css('transition','0.3s');
            }
				...
				//rebuild subcat menu dynamically for mobile display
        var activeHrefMore = $('.more-item.active-trail').attr('href');
        $('ul.navbar-nav li.expanded a.hidden-sm').each(function(index) {
          if($(this).attr('href') == activeHrefMore) {
            $(this).parent().addClass('active');
          }
        });

				$('#navbar .container').once().append('<div class="mobile-menu animated bounceInRight"></div>');
        $subcatCopy = $('#navbar .active .subcat-menu').once();
        $subcatCopy.prepend('<li class="subcat-parent-link"></li>');
        $('.subcat-parent-link').prepend($subcatCopy.prev().clone());

				$subcatCopy.clone().removeClass('subcat-menu').addClass('mobile-subcat-menu visible-xs').appendTo($('.mobile-menu'));
			}
    }
  };

/* floating Prev Next Link */
  Drupal.behaviors.siblingLinks = {
    attach: function (context, settings) {

        $('.floatlinks').show();
        $(window).on('load resize scroll', function() {
          if ($('.field-name-body-linked').length > 0) {
              displayFloatedSibLinks($('.field-name-body-linked'));
          }
        });

        function displayFloatedSibLinks(element) {
          if ($(element).isOnScreen()) {
           $( ".floatlinks" ).show('slow');
          }else{
           $( ".floatlinks" ).hide('slow');
          }
        }
        $.fn.isOnScreen = function(){
          var element = this.get(0);
          var bounds = element.getBoundingClientRect();
          return bounds.top < window.innerHeight && bounds.bottom-200 > 0;
        }
    }
  };
  ...
  Drupal.behaviors.breakpoints = {
    attach: function (context, settings) {
      ...
      breakpoint.refreshValue = function () {
        ...
        $('body').removeClass('mobile-display tablet-display desktop-display');
        $('body').addClass(this.value + '-display');
      };
      $(window).resize(function () {
...
/* faux block links */
  ...
  Drupal.behaviors.faux_block_link = {
    attach: function (context, settings) {
      $( ".pane-content .media" ).each(function( i ) {
        var anchor = null;
        var ExternalUrl = '';
        anchor = $(this).find( ".story-title .story-headline a" );
        if(anchor.length) {
          var blockLink = anchor.attr('href');

          // check if blockLink has external url (refer to views-view-fields--articles.tpl.php)
          if (blockLink === 'javascript:void(0)'){
            ExternalUrl = 'onclick="' + anchor.attr('onclick') + '"';
          }

          var blockLinkTarget = '';
          if (anchor.attr('target') !== undefined && anchor.attr('target') !== false) {
            blockLinkTarget = 'target="'+anchor.attr('target')+'"';
          }
          var blockLinkText = anchor.text();
          $('<a class="block-link" href="'+blockLink+'" '+ExternalUrl+' title="'+blockLinkText+'" '+blockLinkTarget+'></a>').appendTo($(this));
        }
      });

      $(".mode-embed_related_story aside.group-related-story-embed, .mode-embed_related_story_without_pic aside.group-related-story-embed, .mode-embed_horizontal_related_story_q aside.group-related-story-embed").each(function( i ) {
        var anchor = null;
        if ($(this).find("a.block-link").length == 0) {
          anchor = $(this).find(".related-story-headline a");
          if(anchor.length) {
            var blockLink = anchor.attr('href');
            var blockLinkTarget = '';
            if (anchor.attr('target') !== undefined && anchor.attr('target') !== false) {
            ...
            var blockLinkText = anchor.text();
            $('<a class="block-link" href="'+blockLink+'" title="'+blockLinkText+'" '+blockLinkTarget+'></a>').appendTo($(this));
          }
        }
      });

      /* add helper class mobile identifier to html tag */
      if (navigator.userAgent.match(/(iPad|iPhone|iPod)/i)) $('html').addClass('device-ios');
      if (navigator.userAgent.match(/android/i))  $('html').addClass('device-android');
  ...
  Drupal.behaviors.equalheight_columns = {
    attach: function (context, settings) {
      ...
      equalheight = function(container){
        ...
        $(container).each(function() {
          ...
          $($el).removeAttr( "style" );
          $($el).find('.media').removeAttr( "style" );

          topPostion = $el.offset().top;

          var elheight = $el.height();
          if (currentRowStart != topPostion) { // new row
            $el.parents('.classy-panel-styles-pane').addClass('firstOfRow no-border');
            ...
            $el.parents('.classy-panel-styles-pane').removeClass('firstOfRow no-border');
            rowDivs.push($el);
            currentTallest = ((currentTallest < elheight) && (elheight<1000)) ? (elheight) : (currentTallest);
          }
          for (currentDiv = 0 ; currentDiv < rowDivs.length ; currentDiv++) {
            totalMediaHeight = 0;
            mediaCount = 0;
            if(rowDivs[currentDiv].find('.media').length == 1 ){
              rowDivs[currentDiv].find('.media').height(currentTallest);
              if (rowDivs[currentDiv].find('.group-liveblog').length == 1){
                if((currentTallest-55)>100){
                  rowDivs[currentDiv].find('.liveblog-block').height(currentTallest-55);
                }
              }
              /*else if (rowDivs[currentDiv].find('.paragames-2015-medal').length == 1){
                if((currentTallest-55)>100){
                  rowDivs[currentDiv].find('.paragames-2015-medal table').height(currentTallest-29);
                }
              }*/
            }else{
              rowDivs[currentDiv].find('.media:not(:last-child)').each(function(){
                totalMediaHeight += $(this).height();
                mediaCount++;
              });
              pad = (mediaCount==1) ? 10 : mediaCount*10;
              if ((currentTallest-totalMediaHeight-pad)>0){
                rowDivs[currentDiv].find('.media:not(.equal-height--negate-media):last-child').height(currentTallest-totalMediaHeight-pad);
              }
            }
          }
        });
        ...
        if (window.innerWidth >= 740) {
          //setTimeout(function() {
          var stttop = document.querySelectorAll('.stt-top1 .media');
          if (stttop.length > 0) {
            var stttop_height = stttop[0].style.height;
            if (document.querySelectorAll('.stt-top1 .media-body')[0].offsetHeight > 386) {
              //
            } else {
              document.querySelectorAll('.stt-top1 .media')[0].style.height = "386px";
              stttop_height = "386px";
              //document.querySelectorAll('.stt-top1 .media')[0].style.height = "auto";
            }

            var breakingNewsDOM = document.querySelectorAll('.pane-articles-breaking-news-consolidated');
            if (breakingNewsDOM.length > 0) {
              breakingNewsDOM[0].style.height = (parseFloat(stttop_height, 10) - 10) + "px";
              document.querySelectorAll('.pane-articles-breaking-news-consolidated .pane-content')[0].style.height = (parseFloat(stttop_height, 10) - 40) + "px";
            }

            var liveblogDOM = document.querySelectorAll('.pane-bundle-24liveblog-widget .group-lb.media');
            if (liveblogDOM.length > 0) {
              liveblogDOM[0].style.height = (parseFloat(stttop_height, 10)) + "px";
            }
          }
          //}, 200);
        } else {
          var breakingNewsDOM = document.querySelectorAll('.pane-articles-breaking-news-consolidated');
          if (breakingNewsDOM.length > 0) {
            breakingNewsDOM[0].style.height = "auto";
            document.querySelectorAll('.pane-articles-breaking-news-consolidated .pane-content')[0].style.height = "auto";
          }
        }
      };
      ...
      $(window).load(function() {
        if ( $('body').hasClass('panels-ipe') ) {
          equalheight('.panels-ipe .region-inner .classy-panel-styles-pane .pane-content');
        } else {
          equalheight('.region:not(.flexybokxy) .region-inner .classy-panel-styles-pane .pane-content');
        }
        //equalheight('.region-inner .classy-panel-styles-pane .pane-content');
        $('.liveblog-block').css('max-height','none');
      });
  ...
  /*
  Drupal.behaviors.subheadlinecss = {
    attach: function (context, settings) {
      var subheadlineelem = document.querySelector('.node-subheadline');
      if (subheadlineelem) {
        var subheadlinetext = subheadlineelem.textContent;
        if ( (subheadlinetext.match(/\n/g)||[]).length > 0 ) {
          var subheadlineHTML = subheadlinetext.replace(/\n/g, "<br>");
          subheadlineelem.innerHTML = subheadlineHTML;
        }
      }
    }
  };
  */
  ...
  Drupal.behaviors.parallax = {
    attach: function (context, settings) {
        ...
        if ($('.article-type-longform .field-name-field-media .field-item img').length > 0) {
             //console.log($('.field-name-field-media img').first().attr('data-src'));
             var mainImg = $('.field-name-field-media img').first();
             var parallaxBlock = $('.article-type-longform .field-name-field-media');
             mainImg.hide();
             //var parallaxBlockOffTop = parallaxBlock.offset().top;
             //parallaxBlock.css('background-position-y', parallaxBlockOffTop + 'px');
             parallaxBlock.css('background-image', 'url("' + mainImg.attr('src') + '")');
             parallaxBlock.css({"opacity":0}).delay(200).css({"opacity":1});

            /*$(window).scroll(function() {
                var y_offset = $(this).scrollTop();
                parallaxBlock.css('background-position-y', (parallaxBlockOffTop - (y_offset * 1.5)) + 'px');
        ...
        if ( $(".article-type-longform .field-name-body .longform-chaplinks").length == 1 ) {
          $(".field-name-body .longform-chaplinks").appendTo("nav[role=navigation]");

          $(".article-type-longform .longform-chaplinks .visbutton").on("click", function() {
  ...
  Drupal.behaviors.longform_render = {
    attach: function (context, settings) {
      ...
      if (document.querySelector('body.article-type-longform')) {
        ...
        !document.querySelector('body.article-type-longform .view-mode-article_long_form .field-name-field-media')) {
            document.querySelector('body.article-type-longform .view-mode-article_long_form').classList.add('banner-color');
            ...
            if(document.querySelector('body.article-type-longform .banner-bg-color').dataset.reverse === 'true') {
                document.querySelector('body.article-type-longform .view-mode-article_long_form').classList.add('header-reverse');
            }
        ...
        window.addEventListener('load', function(event) {
          ...
          document.querySelector('body.article-type-longform .field-name-field-media .field-item:nth-child(2) img') ) {
              document.querySelector('body.article-type-longform .field-name-field-media').style.backgroundImage = 'url(' + document.querySelector('body.article-type-longform .field-name-field-media .field-item:nth-child(2) img').src + ')';
          }
          // Navbar overlap for mobile
          if (window.matchMedia("(max-width: "+ maxWidth +")").matches &&
          !document.querySelector('body.article-type-longform .field-name-field-media .field-item:nth-child(1) img') &&
          ...
          document.querySelector('body.article-type-longform .longform-chaplinks') ) {
            ...
            document.querySelector('body.article-type-longform header#navbar .chaplinks-list').appendChild(document.querySelector('body.article-type-longform header#navbar .longform-social-button'));
            ...
            document.querySelector('body.article-type-longform .longform-chaplinks .visbutton ').onclick = function(){
                if(document.querySelector('body.article-type-longform nav .longform-chaplinks.active')) {
            ...
            if (document.querySelector('.highlighted.hidden-print')) {
              document.querySelector('.longform-chaplinks .chaplinks-list').style.height =  window.innerHeight - document.querySelector('.highlighted.hidden-print').clientHeight + 'px';
              document.querySelector('.longform-chaplinks .chaplinks-list').style.top =  (document.querySelector('.highlighted.hidden-print').clientHeight + 2) + 'px';
            }
          }
          ...
          // video hero banner
          if (document.querySelector('body.article-type-longform .field-name-body .herobanner-video')) {
            if(document.querySelector('body.article-type-longform .view-mode-article_long_form > .row > div > .field-name-field-media > .field-items .field-item')) {
              document.querySelector('body.article-type-longform .view-mode-article_long_form > .row > div > .field-name-field-media').style.display = 'none';
            }
            document.querySelector('body.article-type-longform .view-mode-article_long_form > .row > div').insertBefore(document.querySelector('body.article-type-longform .field-name-body .herobanner-video'), document.querySelector('body.article-type-longform .group-longform-content'));

            if(document.querySelector('body.article-type-longform .herobanner-video').dataset.height) {
              document.querySelector('body.article-type-longform .herobanner-video').style.height = document.querySelector('body.article-type-longform .herobanner-video').dataset.height + 'px';
            }
            if (window.matchMedia("(max-width: 740px)").matches) {
              document.querySelector('body.article-type-longform header#navbar').classList.remove('navbar-blue');
              document.querySelector('body.article-type-longform .herobanner-video').style.height = Number(window.innerHeight - (document.querySelector('.highlighted.hidden-print').offsetHeight + document.querySelector('.headline').offsetHeight ) ) + 'px';
            }
          }
          ...
          //
          //
          //   var vueComponents = document.querySelectorAll('body.article-type-longform .field-name-body [data-vue-component]');
          //
          ...
          //    if (typeof window._IMPORTS_CACHE_ === 'undefined')  window._IMPORTS_CACHE_ = []
          //     window._IMPORTS_CACHE_ = []
          //     return function(resolve, reject) {
          //       window._IMPORTS_CACHE_[url] =
          //         window._IMPORTS_CACHE_[url] ||
          //         fetch(url)
          //           .then(function(res) {
          //             return res.text();
        ...
        function getBannerColorHeight() {
          document.querySelector('body.article-type-longform .banner-bg-color').style.height = document.querySelector('body.article-type-longform .view-mode-article_long_form.banner-color .node-header').clientHeight + document.querySelector('body.article-type-longform .view-mode-article_long_form.banner-color .group-story-timestamp').clientHeight + parseInt(window.getComputedStyle(document.querySelector('body.article-type-longform .view-mode-article_long_form.banner-color .group-longform-content'), null).getPropertyValue('padding-top')) + 'px';
        }

        if (document.querySelector('body.article-type-longform')) {
          // table
          var tables = document.querySelectorAll('body.article-type-longform .field-name-body table');
          ...
          // remove br from photogrid
          // var br = document.querySelectorAll('.photogrid .photogrid-wrapper br');
          // for (var i = 0; i < br.length; i++) {
          //   br[i].parentNode.removeChild(br[i]);
          // }
          // var p = document.querySelectorAll('.photogrid .photogrid-wrapper p');
          // for (var i = 0; i < p.length; i++) {
          //   p[i].parentNode.removeChild(p[i]);
          // }


          // collapsible sidebar textbox for all articles types
          var sidebarCollapsible = document.querySelectorAll('body.node-type-article .field-name-body .sidebar-textbox');
          for (var i = 0; i < sidebarCollapsible.length; i++) {
            if (sidebarCollapsible[i].querySelector('.expandable') &&
            sidebarCollapsible[i].querySelector('.expandable').innerHTML !== '') {
              if (sidebarCollapsible[i].querySelector('.expandable').innerHTML === '&nbsp;') {
                sidebarCollapsible[i].querySelector('.expandable').innerHTML = '';
              } else {
                sidebarCollapsible[i].querySelector('.expandable').addEventListener('click', function(event) {
  ...
  Drupal.behaviors.market_ticker = {
    attach: function (context, settings) {
      if($(".market-ticker").length) {
        $(".market-ticker").marquee({
          duration: 20 * $('.market-ticker').width(),
          gap: 10,
          delayBeforeStart: 2,
          direction: "left",
          pauseOnHover: true,
  ...
  Drupal.behaviors.fadeshower = {
    attach: function (context, settings) {
      $(document).ready(function() {
        $('.media-group').each(function(ind,elem) {
          var fadecount = $(elem).children('ul').children('li').length;
          //console.log("fadecount: "+ fadecount );
          if (fadecount > 6) {
  ...
  Drupal.behaviors.uadetect = {
    attach: function (context, settings) {
      var domElemList = [".group-liveblog-heading",".pulsedotsvg",".pulsedot",".pulsedot > span"];
      $(document).ready(function() {
        if (navigator.userAgent) {
          var clientplatform = (navigator.userAgent).toLowerCase();
          var isMac = clientplatform.indexOf("macintosh") > -1? true: false;
          var isAndroid = clientplatform.indexOf("android") > -1? true: false;
          var isIE = clientplatform.indexOf("trident") > -1? true: false;
          var isFF = clientplatform.indexOf("firefox") > -1? true: false;
          var isSafari = false;
          if ((clientplatform.indexOf("safari") > -1) && (clientplatform.indexOf("chrome") == -1)) {
            isSafari = true;
          }

          if (isMac) {
            domElemList.forEach(function(elem) {
              $(elem).addClass("mac");
            });
            document.querySelector('body').classList.add('ua-mac');
          }
          if (isAndroid) {
            domElemList.forEach(function(elem) {
              $(elem).addClass("android");
            });
            document.querySelector('body').classList.add('ua-android');
          }
          if (isIE) {
            domElemList.forEach(function(elem) {
              $(elem).addClass("ie");
            });
            document.querySelector('body').classList.add('ua-ie');
          }
          if (isSafari) {
            document.querySelector('body').classList.add('ua-safari');
          }
          if (isFF) {
            document.querySelector('body').classList.add('ua-ff');
          }
        }
      });
    }
  ...
  // REPLACE THE ORIGINAL BOREALIS FUNCTION
  Drupal.behaviors.borealis_ri = {
    attach: function (context, settings) {
      //console.log(settings);
      if (typeof settings.borealis_ri == "undefined") {
        return;
      }

      $(document).ready(function() {

        var windowWidth_curr = document.body.clientWidth;
        ...
        // When the screen is resized, check again! (Smarter)
        window.onresize = debounce(function(){
          var windowWidth_temp = document.body.clientWidth;
          if (windowWidth_temp > windowWidth_curr) {
            if (lazyload) {
              borealisImagePlaceholder();
            } else {
              borealisImageRespond();
            }
            windowWidth_curr = windowWidth_temp;
          }
        }, 20);

        // When the screen is scrolled, do it again! (Smarter)
        window.onscroll = debounce(function(){
          if (lazyload) {
            borealisImagePlaceholder('scroll');
          } else {
            borealisImageRespond();
          }
        }, 20);
        ...
        function borealisImagePlaceholder(isScroll) {
          //console.log('borealisImagePlaceholder augment:', isScroll);

          var images;
          if (isScroll) {
            // minimise the subsequent for loop by getting only blank images
            images = document.querySelectorAll('img.borealis[src="/sites/all/modules/contrib/borealis/borealis_ri/images/blank.png"]');
          } else {
            images = document.querySelectorAll('img.borealis');
          }
        ...
        function borealisImageSwap(img, index, width, bypass) {
          ...
          if (dpr > 1.5 && dpr <= 3.5) {
            ...
            if (styleArray.length <= index + 1) {
              //console.log("handle index out of range!");
          ...
          if (styleArray.indexOf(style) > styleArray.indexOf(imgStyle)) {
            ...
            //console.log('swap image augment');
          }
          else {
            //console.log('no need swap image augment');
  ...
  Drupal.behaviors.pulsedotsvg = {
    attach: function (context, settings) {
      var mediabodytitle = document.querySelectorAll(".live-item-title .media-body"); //.inserthere

      function createPulsedotSVG(svgID) {
      var divsvg = document.createElement('div'); divsvg.setAttribute('class','pulsedotsvg jsadd');

      var svg = document.createElementNS('http://www.w3.org/2000/svg','svg');
      svg.setAttribute('xmlns','http://www.w3.org/2000/svg');
      svg.setAttribute('height','20px'); svg.setAttribute('width','20px'); svg.setAttribute('viewBox','0 0 14 14');

      var animate1 = document.createElementNS('http://www.w3.org/2000/svg','animate');
      animate1.setAttributeNS("http://www.w3.org/1999/xlink", "href", "#pulsedotback"+svgID);
      animate1.setAttribute('attributeName','r');
      animate1.setAttribute('from',3); animate1.setAttribute('to',6);
      animate1.setAttribute('dur', '1s'); animate1.setAttribute('begin', '0s');
      animate1.setAttribute('repeatCount', 'indefinite'); animate1.setAttribute('fill', 'freeze'); animate1.setAttribute('id', 'circ-anim');

      var animate2 = document.createElementNS('http://www.w3.org/2000/svg','animate');
      animate2.setAttributeNS("http://www.w3.org/1999/xlink", "href", "#pulsedotback"+svgID);
      animate2.setAttribute('attributeType','CSS');
      animate2.setAttribute('attributeName','opacity');
      animate2.setAttribute('from',1); animate2.setAttribute('to',0);
      animate2.setAttribute('dur', '1s'); animate2.setAttribute('begin', '0s');
      animate2.setAttribute('repeatCount', 'indefinite'); animate2.setAttribute('fill', 'freeze'); animate2.setAttribute('id', 'circ-anim');

      var circle1 = document.createElementNS('http://www.w3.org/2000/svg','circle');
      circle1.setAttribute('class','pulsedotback');
      circle1.setAttribute('id','pulsedotback'+svgID );
      circle1.setAttribute('cx','7.2'); circle1.setAttribute('cy','6.8');
      circle1.setAttribute('r','3'); circle1.setAttribute('stroke-width','1');

      var circle2 = document.createElementNS('http://www.w3.org/2000/svg','circle');
      circle2.setAttribute('class','pulsedotfront');
      circle2.setAttribute('cx','7.2'); circle2.setAttribute('cy','6.8');
      circle2.setAttribute('r','2.1');

      svg.appendChild(animate1); svg.appendChild(animate2); svg.appendChild(circle1); svg.appendChild(circle2);
      divsvg.appendChild(svg);
        return divsvg;
      }

      var mediamediabodytitleLen = mediabodytitle.length;
      for (var i = 0; i < mediamediabodytitleLen; i++) {
        mediabodytitle[i].appendChild( createPulsedotSVG('dotter'+i) );
      }
    }
  };
...
/*
      window.onload = function(){
        $('.with-social-count .social-count, body.node-type-author .social-count').each(function(index) {
          var nid = $(this).data('social-nid');
          nid = ".social-count-" + nid;
          $.ajax({
            type: "GET",
            url: "/social-stats-count-a2a/get?nid=" + $(this).data('social-nid'),
            cache: true,
            timeout: 10000,
 ...
 Drupal.behaviors.lifestyleCarousel = {
  attach: function(context, settings){
    /*if ($('.instagram-gallery').length > 0) {
      $(".instagram-gallery .view-content").owlCarousel({
        items : 6,
        itemsDesktop : [1199,6], //6 items between 1000px and 901px
        itemsDesktopSmall : [979,4], // betweem 991px and 601px
